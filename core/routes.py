...
from core.utlis import get_horoscope_by_day,get_horoscope_by_week,get_horoscope_by_month
...
class MonthlyHoroscopeAPI(Resource):
     '''show weekly horoscope of zodiac sign'''
     @ns.doc(parser=parser)
     def get(self):
         args=parser.parse_args()
         zodiac_sign=args.get('sign')
         try:
             ...
             raise NotFound('no such ziodic sign exist')
         except AttributeError:
             raise BadRequest('Something went wrong, please check the URL and the arguments.')
